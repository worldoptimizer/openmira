...
- **Done:** Resource infrastructure is now real for project-map summary and memory snapshot. Screenshot image resources were later removed in v1.3.0 because client support did not deliver useful agent-visible image content in Claude Code CLI.
...
- **Done:** Pilot #13 originally validated screenshot resources in a controlled run, but later dogfood testing showed Claude Code CLI could not reliably consume those images as model-visible vision input. v1.3.0 removes that delivery path.
...
- **Now:** Prepare the 1.3.0 release: build an installable ZIP from the cleaned screenshot surface, tag `1.3.0`, and attach the built ZIP to the GitHub Release.
...
8. **Done, v1.3 cleanup:** External screenshot URL jobs — same-site jobs persist viewport metadata and the Playwright bridge stores PNG/JPEG files on disk. Removed browser queue, inline base64, and MCP screenshot delivery after dogfood testing showed they did not deliver agent-visible images in Claude Code CLI.
...
13. **Done, v1:** Resource infrastructure and targeted Gutenberg reliability — project-map and memory snapshots register JSON resources; testimonial and feature-grid pattern rendering preserve the Pilot #12 design metadata. Screenshot image resources were removed in v1.3.0.
