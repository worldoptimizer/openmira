...
- **Done:** Screenshot subsystem was reduced for v1.4.0: `screenshot-url` creates same-site jobs for the external Playwright bridge, `complete-screenshot-url-job` stores PNG/JPEG files under `wp-content/openmira-screenshots/`, and agent-facing image delivery paths were removed because Claude Code CLI cannot reliably consume them as vision input.
...
- **Done:** Resource infrastructure is now real for project-map summary and memory snapshot. Screenshot image resources were later removed in v1.4.0 because client support did not deliver useful agent-visible image content in Claude Code CLI.
...
- **Done:** Pilot #13 originally validated screenshot resources in a controlled run, but later dogfood testing showed Claude Code CLI could not reliably consume those images as model-visible vision input. v1.4.0 removes that delivery path.
...
- **Done:** Test 3B screenshot friction led to a later rollback of inline image delivery. the old public screenshot image-delivery ability was removed; a private bridge-only metadata helper remains for the Playwright script.
...
- **Now:** Prepare the 1.4.0 cleanup release: remove overbuilt screenshot runner/image-delivery infrastructure, keep external capture-to-disk support, build an installable ZIP, tag `1.4.0`, and attach the built ZIP to the GitHub Release.
...
- **External screenshot capture**: `screenshot-url` creates a same-site job, and `scripts/openmira-complete-screenshot-job.sh` captures it with Playwright and stores the result under `wp-content/openmira-screenshots/`. Direct agent-visible image delivery is deferred until MCP clients reliably expose native image content to the model.
...
8. **Done, v1.4 cleanup:** External screenshot URL jobs — same-site jobs persist viewport metadata and the Playwright bridge stores PNG/JPEG files on disk. Removed browser queue, inline base64, and MCP screenshot-image delivery after dogfood testing showed they did not deliver agent-visible images in Claude Code CLI.
...
13. **Done, v1:** Resource infrastructure and targeted Gutenberg reliability — project-map and memory snapshots register JSON resources; testimonial and feature-grid pattern rendering preserve the Pilot #12 design metadata. Screenshot image resources were removed in v1.4.0.
14. **Done, historical:** Pilot #13 originally used screenshot resources successfully in a controlled benchmark. Later dogfood testing refuted that as a reliable Claude Code CLI path, so screenshot resources were removed.
