...
- Create at least one screenshot job, complete it with the bridge, then prefer the returned `resource_uri` / MCP Resource channel or bridge screenshot file for visual review. Do not request `include_image: true` unless Resource reading and local file review are both unavailable.
...
- No screenshot read uses `include_image: true` unless explicitly reported as a client limitation.
...
- Whether `include_image: true` was used.
